...
“”“استخراج حركات البنك من PDF”””
data = []
with pdfplumber.open(file) as pdf:
for page in pdf.pages:
# محاولة استخراج جداول أولاً
...
“”“استخراج كشف الحساب وتحديد نوع كل حركة”””
if file.name.endswith(’.pdf’):
raw = extract_pdf_transactions(file)
if raw.empty:
return pd.DataFrame()
# محاولة تفسير الأعمدة
df = raw.copy()
...
if order_file and bank_file:
    ...
    status_filter = st.multiselect(
        "اختر الحالة", result["Status"].unique().tolist() if "Status" in result.columns else [],
...
if stmt_file:
    ...
    if df_raw.empty:
        ...
        if st.button("🔍 تحليل الكشف"):
            ...
            # إعادة تسمية الأعمدة
            rename = {}
            if date_col != "—": rename[date_col] = "التاريخ"
            if desc_col != "—": rename[desc_col] = "الوصف"
